...
export class CementPlantSimulator {
  ...
  generateEquipmentStatus(): EquipmentStatus[] {
    ...
    return equipment.map(eq => ({
      ...
      status: Math.random() > 0.05 ? 'running' : Math.random() > 0.5 ? 'maintenance' : 'fault',
      operating_hours: this.getRandomInRange(1000, 8760), // Annual hours
      efficiency: this.getRandomInRange(85, 98),
      last_maintenance: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // Last 30 days
      next_maintenance: new Date(Date.now() + Math.random() * 60 * 24 * 60 * 60 * 1000), // Next 60 days
      alerts: this.generateAlertsForEquipment(eq.id),
